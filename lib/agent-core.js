...
import { hasDateHint } from './parser.js';
...
// The AI-button brain: the smart model with a small prompt. Haiku was tried
// live (2026-08-26) and rejected — clunky Hebrew, chatty clarification loops,
// and it invented restaurant values because it had no web search. The button
// is rare and budget-capped, so the strong model costs agorot either way.
const LITE_MODEL = process.env.AGENT_LITE_MODEL || 'claude-sonnet-5';
const MAX_ROUNDS = 3;        // tool-executing Claude calls per message (Hobby: 60s hard cap)
const MAX_PAUSES = 4;        // server-tool (web search) continuations, budgeted apart from rounds
...
/* Record what a message cost (best-effort) — every route, not just Claude,
   so /cost can show how much of the traffic was handled for free. */
export async function logUsage(row) {
  const { error } = await sb.from('agent_usage').insert(row);
  if (error) console.error('usage log error:', error.message || error);
}
...
function foodsBlock(foods) {
  ...
  return foods.map((f) => {
    const partial = f.kcal_per_100g == null && f.package?.kcal == null;
    let s = `- "${f.alias}"${partial ? ' ⚠️חסרים ערכים' : ''}`;
    if (f.aliases?.length) s += ` (גם: ${f.aliases.join(', ')})`;
    if (f.product) s += ` = ${f.product}`;
    if (f.package?.kcal != null) {
      s += ` · אריזה שלמה (${f.package.unit || 'אריזה'}): ${f.package.kcal} קק"ל`;
      if (f.package.grams) s += `, ${f.package.grams} גרם`;
    }
...
11. למידה אוטומטית: כל מאכל שרשמת ואינו כבר במילון — קרא גם ל-remember_food, ותמיד עם aliases (2-4 שמות נוספים שהמשתמש עשוי לומר לאותו דבר). גם תיקון כמות או זיהוי מוצר נשמרים כך. פריטים נפרדים = רשומות נפרדות. כשלמדת — אמור זאת במשפט הסיכום הקצר.
    ...
    input_schema: {
      ...
      properties: {
        ...
        aliases: {
          type: 'array',
          items: { type: 'string' },
          description: 'שמות נוספים סבירים שהמשתמש עשוי לומר לאותו מאכל, 2-4 וריאציות. למשל עבור "מק דאבל": ["מקדונלדס דאבל","דאבל","המבורגר מק דאבל"]. אל תכלול את alias עצמו.',
        },
        product: { type: 'string', description: 'שם המוצר המלא, למשל "קוטג\' תנובה 5%"' },
        package: {
          type: 'object',
          description: 'למוצר ארוז/מנה קבועה (המבורגר, גביע, בקבוק, חטיף) — הערכים של האריזה השלמה מהתווית. כשהמשתמש יאמר את שם המאכל בלי כמות, זו הכמות שתירשם. אל תשלח למאכל שנמדד במשקל (אורז, בשר, ירקות).',
          properties: {
            grams: { type: 'number', description: 'משקל האריזה בגרמים, אם ידוע' },
            kcal: { type: 'number', description: 'קלוריות באריזה השלמה' },
            protein: { type: 'number' }, carbs: { type: 'number' }, fat: { type: 'number' },
            unit: { type: 'string', description: 'שם היחידה: "מנה" / "גביע" / "בקבוק" / "יחידה"' },
          },
          required: ['kcal'],
        },
...
// The lite toolset: what the AI button actually needs — turn a phrasing into a
// logged meal. Web search rides along for restaurant/brand values (the tool
// variant is unsupported on Haiku — gate it, in case the env overrides).
const LITE_TOOL_NAMES = new Set(['log_meal', 'update_meal', 'remember_food', 'lookup_israeli_food']);
const TOOLS_LITE = TOOLS.filter((t) => LITE_TOOL_NAMES.has(t.name));
if (!/haiku/.test(LITE_MODEL)) {
  TOOLS_LITE.push({ type: 'web_search_20260209', name: 'web_search', max_uses: 2 });
}

/* The lite prompt — a fraction of the full one. Only what the button's job
   needs: decode one odd phrasing into a meal, with provenance rules intact. */
function buildSystemLite(ctx) {
  return `אתה מפענח רישומי אוכל בעברית לבוט תזונה (משתמש יחיד). המשתמש שלח ניסוח שהקוד לא זיהה — תפקידך להבין ולרשום.

חוקים:
1. סדר עדיפות לערכים: המילון האישי → lookup_israeli_food (מאגר משרד הבריאות) → הערכה שלך, בקצה הגבוה. לכל פריט source_type: personal_food / israeli_db / ai_estimate, ו-quantity_source: user_explicit / default / estimated.
1א. מנת מסעדה/רשת או מוצר ממותג (מקדונלדס, ברגר קינג וכו') — חפש ב-web_search את הערכים הרשמיים. לעולם אל תמציא ערכים למותג. מצאת — source_type="label" וציין את המקור ב-assumptions.
1ב. **היסטוריית השיחה אינה מקור לערכים.** מספרים שמופיעים בהודעות קודמות שלך הם רק תזכורת למה שנרשם — לא נתון מאומת. לכל רישום חדש גזור ערכים מחדש מהמילון, מהמאגר או מחיפוש. אל תעתיק קלוריות מהודעה קודמת בשיחה.
1ג. **תאריך: תמיד היום ועכשיו**, אלא אם המשתמש אמר במפורש בהודעה הנוכחית מתי ("אתמול", "בצהריים", שעה). מה שנאמר בהודעות קודמות על תאריכים לא חל על הרישום הזה.
2. רישום חדש = log_meal. תיקון לארוחה מהיום = update_meal עם הפירוק המלא — פריטים שלא השתנו מועתקים מההקשר מילה-במילה, בלי הערכה מחדש.
3. **חובה: כל מאכל שרשמת ואינו כבר במילון — קרא גם ל-remember_food.** זה הכלל החשוב ביותר כאן: אתה נקרא רק כשהקוד לא הצליח, ותפקידך לוודא שבפעם הבאה הוא כן יצליח בלי לקרוא לך.
   - alias = איך המשתמש קרא לזה בהודעה שלו, בדיוק.
   - aliases = 2-4 שמות נוספים סבירים שהוא עשוי לומר בעתיד לאותו דבר (קיצור, שם מלא, שם המותג, וריאציה בכתיב).
   - product = השם המסחרי/המלא. serving_grams = משקל המנה הרגילה. וכל ערכי ה-100 גרם שידועים לך.
   - **מוצר ארוז או מנה קבועה** (המבורגר, גביע, בקבוק, חטיף, מעדן) — שלח גם package עם ערכי האריזה השלמה מהתווית. זו תהיה ברירת המחדל כשהמשתמש יאמר את השם בלי כמות. למאכל שנמדד במשקל (אורז, בשר, ירקות) אל תשלח package.
   - גם מנת מסעדה ומוצר ממותג נשמרים — הם חוזרים.
   - פריטים נפרדים = רשומות נפרדות (המבורגר וצ'יפס = שתי קריאות), לא רשומה אחת מאוחדת.
4. אחרי פעולת כתיבה הקוד מציג אישור מלא — ענה טקסט ריק, או משפט אחד רק אם יש מידע שהאישור לא מראה. אם לא ביצעת כתיבה — חובה לענות בעברית, קצר.
5. עיצוב: שורה לרעיון, המספר החשוב מודגש <b>כך</b>. מותר רק <b> <i> <code> — בלי markdown, בלי כוכביות. אחרי כתיבה אל תחזור על המספרים שבאישור.
6. טון: עברית טבעית, ישיר וקצר. בלי הקדמות ("עדכון:", "מובן —"). כשחסר מידע — שאלה אחת שמאגדת את כל מה שחסר, לא סבב שאלות.
...
עכשיו: ${dayKey()} ${israelNowHM()} (שעון ישראל).
אם המאכל שביקשו לרשום כבר מופיע ברשימת היום — שאל אם זו מנה נוספת לפני שתרשום שוב.`;
}
...
export async function execLogMeal(chatId, rawText, input) {
...
export async function execUpdateMeal(chatId, input) {
  ...
  const patch = { items: input.items, totals, assumptions: input.assumptions || '', confidence: input.confidence };
  // Backdating ("זה היה אתמול"): move the meal, keep its clock time. The prev
  // snapshot gains the old date so undo restores it wholesale.
  if (input.date && input.date !== meal.day_key) {
    patch.day_key = input.date;
    patch.ts = `${input.date}${String(meal.ts).slice(10)}`;
    prev.day_key = meal.day_key;
    prev.ts = meal.ts;
  }
  ...
  return {
    ...
    items: input.items, totals, assumptions: input.assumptions || '',
    confidence: input.confidence, dayKeyUsed: patch.day_key || meal.day_key,
  };
}

export async function execDeleteMeal(chatId, input) {
  const { data: meal, error: e1 } = await sb
    .from('meals').select('*').eq('id', input.meal_id).eq('chat_id', chatId).maybeSingle();
  if (e1) throw e1;
  if (!meal) return { ok: false, error: 'meal not found' };
...
const FOOD_FIELDS = ['product', 'barcode', 'aliases', 'package', 'serving_grams', 'kcal_per_100g', 'protein_per_100g', 'carbs_per_100g', 'fat_per_100g', 'variants'];

/* The model reaches for a nested per-100g object even though the schema is
   flat, and unknown keys were dropped in silence — the entry got saved with a
   name and no calories, so the parser could never use it and the next mention
   cost another API call. Accept both shapes. */
const PER100_KEYS = {
  calories: 'kcal_per_100g', kcal: 'kcal_per_100g', energy: 'kcal_per_100g',
  protein: 'protein_per_100g', carbs: 'carbs_per_100g',
  carbohydrates: 'carbs_per_100g', fat: 'fat_per_100g',
};

export function normalizeFoodInput(input = {}) {
  const out = { ...input };
  const nested = input.per_100g || input.per100g || input.per_100 || input.nutrition;
  if (nested && typeof nested === 'object') {
    for (const [k, v] of Object.entries(nested)) {
      const field = PER100_KEYS[String(k).toLowerCase()];
      if (field && out[field] === undefined && Number.isFinite(Number(v))) out[field] = Number(v);
    }
  }
  for (const k of ['per_100g', 'per100g', 'per_100', 'nutrition']) delete out[k];
  return out;
}

export async function execRememberFood(chatId, rawInput) {
  const input = normalizeFoodInput(rawInput);
  ...
  // A NEW entry with no energy value is useless to the parser and will
  // silently cost another API call on the next mention — refuse it so the
  // model sends the numbers. Updates to an existing entry stay partial.
  if (!prev) {
    const hasEnergy = Number.isFinite(Number(input.kcal_per_100g)) || Number.isFinite(Number(input.package?.kcal));
    if (!hasEnergy) {
      return { ok: false, error: 'חסר kcal_per_100g (או package.kcal) — רשומה חדשה בלי ערך אנרגיה לא ניתנת לשימוש. שלח שוב עם הערכים.' };
    }
  }
  ...
  // Alternative names accumulate rather than replace: a later mention teaching
  // one new way to say it must not erase the ways already learned.
  if (row.aliases || prev?.aliases) {
    const seen = new Set();
    const out = [];
    for (const a of [...(prev?.aliases || []), ...(row.aliases || [])]) {
      const nm = String(a || '').trim();
      const k = nm.toLowerCase();
      if (!nm || k === alias.trim().toLowerCase() || seen.has(k)) continue;
      seen.add(k);
      out.push(nm);
    }
    row.aliases = out.slice(0, 8);
  }
...
export async function execLogMeasurement(chatId, input) {
...
export const scaleItem = (it, factor) => {
  const out = { ...it };
...
export async function execLogSaved(chatId, rawText, input) {
  const name = (input.name || '').trim();
  ...
  return execLogMeal(chatId, rawText, { items, confidence: 'high', assumptions: note, ...(input.date ? { date: input.date } : {}) });
}
...
export async function execLookupBarcode(chatId, input) {
...
export async function execLookupIsraeliFood(chatId, input) {
...
async function callClaude(system, messages, { forceText = false, noTools = false, lite = false } = {}) {
  const res = await fetch('https://api.anthropic.com/v1/messages', {
    ...
    body: JSON.stringify({
      model: lite ? LITE_MODEL : MODEL, max_tokens: 4000,
      system, messages,
      ...(noTools ? {} : { tools: lite ? TOOLS_LITE : TOOLS }),
...
const textOf = (resp) => (resp?.content || [])
...
export async function runAgent(chatId, userContent, ctx, rawText, opts = {}) {
  const lite = !!opts.lite;
  const started = Date.now();
  const system = lite ? buildSystemLite(ctx) : buildSystem(ctx);
  ...
  // A trailing user turn with no stored answer would break role alternation.
  // Never invent an assistant turn that says it did not reply — that teaches the
  // model that silence is an acceptable response and the failure repeats. Fold
  // the unanswered text into the current message instead.
  let content = userContent;
  if (history.length && history[history.length - 1].role === 'user') {
    const dangling = history.pop().content;
    content = typeof content === 'string'
      ? `${dangling}\n${content}`
      : [...content, { type: 'text', text: `ההודעה הקודמת שלי: ${dangling}` }];
  }

  const messages = [...history, { role: 'user', content }];
  const mealText = rawText || (typeof userContent === 'string' ? userContent : '📷 תמונה');
  const dateHinted = hasDateHint(rawText || '');
  const actions = [];
  let finalText = '';

  // Rounds and pauses are budgeted separately: a pause_turn is the web-search
  // tool asking to keep going, not a reasoning round, and letting it eat the
  // round budget used to end the turn with nothing at all to show the user.
  let rounds = 0;
  let pauses = 0;
  let lastStop = null;
  const toolErrors = [];

  // Token accounting, straight from the API's own usage report. Summed across
  // every call this turn makes — loop rounds and retries alike.
  const usage = { input: 0, output: 0, cacheWrite: 0, cacheRead: 0, calls: 0 };
  const addUsage = (resp) => {
    const u = resp?.usage || {};
  ...
  while (rounds < MAX_ROUNDS && pauses < MAX_PAUSES) {
    if (Date.now() - started > DEADLINE_MS) { lastStop = 'deadline'; break; }
    const resp = addUsage(await callClaude(system, messages, { lite }));
    lastStop = resp.stop_reason || 'none';

    // Keep the last non-empty answer: a later round that says nothing must not
    // erase an answer an earlier round already produced.
    const said = textOf(resp);
    ...
    for (const tu of toolUses) {
      let result;
      try {
        // A date the user did not ask for is a bug, not a feature: the rolling
        // conversation log carries older turns that mentioned yesterday, and
        // the model backdated a meal on the strength of one. If nothing in the
        // current message names a time, the meal belongs to now.
        if ((tu.name === 'log_meal' || tu.name === 'update_meal') && !dateHinted) {
          delete tu.input.date;
          delete tu.input.meal_time;
        }
      ...
      if (!result.ok) toolErrors.push(`${tu.name}: ${result.error}`);
  ...
  // end for the user. Two escalating retries — the first keeps the conversation
  // and only takes the tools away, the second throws the conversation away too,
  // because a poisoned history is the likeliest reason the model went silent.
  // Deliberately not gated on the deadline: an answer late beats no answer.
  if (!actions.length && !finalText) {
    try {
      finalText = textOf(addUsage(await callClaude(system, messages, { forceText: true, lite })));
    } catch (err) {
      console.error('forced-text retry failed:', err.message);
      toolErrors.push(`retry: ${err.message.slice(0, 120)}`);
    }
    if (!finalText) {
      try {
        finalText = textOf(addUsage(await callClaude(system, [{ role: 'user', content }], { noTools: true, lite })));
      } catch (err) {
        console.error('bare retry failed:', err.message);
        toolErrors.push(`bare: ${err.message.slice(0, 120)}`);
      }
    }
  }
  ...
  return {
    actions,
    text: finalText,
    usage: { ...usage, model: lite ? LITE_MODEL : MODEL },
    diag: { stop: lastStop, rounds, pauses, ms: Date.now() - started, toolErrors },
  };
}
