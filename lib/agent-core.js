...
async function callClaude(system, messages, { forceText = false, noTools = false } = {}) {
...
export async function runAgent(chatId, userContent, ctx, rawText) {
  ...
  while (rounds < MAX_ROUNDS && pauses < MAX_PAUSES) {
    ...
    const resp = addUsage(await callClaude(system, messages));
  ...
  if (!actions.length && !finalText) {
    try {
      finalText = textOf(addUsage(await callClaude(system, messages, { forceText: true })));
    } catch (err) {
    ...
    if (!finalText) {
      try {
        finalText = textOf(addUsage(await callClaude(system, [{ role: 'user', content }], { noTools: true })));
      } catch (err) {
