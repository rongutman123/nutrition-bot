...
export function parseMealText(text, ctx = {}) {
  ...
  if (segments.length === 1 && unknown.length === 1) {
    ...
    const g = seg.match(/(\d+(?:\.\d+)?)\s*(?:גרם|גר|ג)(?=\s|$)/);
    // "פילה דג באסה בתנור - 450 גרם" leaves a dangling separator once the
    // weight is cut out; it would go into the search query and the alias.
    const name = (g
      ? (seg.slice(0, g.index) + ' ' + seg.slice(g.index + g[0].length))
      : seg)
      .replace(/\s+/g, ' ')
      .replace(/^[\s\-–—:•]+|[\s\-–—:•]+$/g, '')
      .trim();
    // Real dish names run longer than three words ("פילה דג באסה בתנור"), so
    // the cap is generous — but a sentence is not a dish name, and searching
    // the database for one produces nonsense. Verbs and hedges give it away.
    const words = name.split(' ').filter(Boolean);
    const isSentence = new RegExp(
      `${E}(?:אכלתי|שתיתי|לקחתי|היה|הייתי|משהו|אולי|בערך|קצת|נראה|חושב|זוכר|אחרי|לפני|כנראה|נדמה)${D}`
    ).test(name);
    if (name.length >= 2 && words.length <= 6 && !isSentence && !/\d/.test(name)) {
