...
export function findFood(name, foods = []) {
  ...
  // exact alias wins over everything, then exact product, then fuzzy
  for (const f of foods) if (key(f.alias) === q) return f;
  for (const f of foods) if (f.product && key(f.product) === q) return f;
  for (const f of foods) if (namesMatch(f.alias, q)) return f;
  for (const f of foods) if (f.product && namesMatch(f.product, q)) return f;
  return null;
}
...
export function parseSegment(segment, foods) {
  ...
  if (!food || food.kcal_per_100g == null) return null;
